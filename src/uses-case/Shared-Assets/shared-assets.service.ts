...
import {DeleteSharedDto} from "./DTO/DeleteShared.dto";
...
export class SharedAssetsService {
    ...
    async updateacceslevelbyUser(updateSharedDto: updateSharedDto) {
        const sharedAssets = await this.sharedModel.find({userid: updateSharedDto.userID,}).exec();


        if (updateSharedDto.docID) {
            const sharedAsset = sharedAssets.find(asset => asset.docid == updateSharedDto.docID);
            if (sharedAsset) {
                sharedAsset.acceslevel = updateSharedDto.acceslevel;
                await sharedAsset.save();
                return sharedAsset.acceslevel;
            }
        } else if (updateSharedDto.foldID) {
            const sharedAsset = sharedAssets.find(asset => asset.folderid == updateSharedDto.foldID);
            if (sharedAsset) {
                sharedAsset.acceslevel = updateSharedDto.acceslevel;
                await sharedAsset.save();
                return sharedAsset.acceslevel;
  ...
  async findsfolderanddocbyUser(findsharedDto: findSharedDto) {
    const allfolderIds = (await this.folderModel.find({ createdby: findsharedDto.Userid })).map(folder => folder.id);
    const sharedfolder = await this.sharedModel.find({ folderid: { $in: allfolderIds } });
    const userIds = sharedfolder.map(sharedf => sharedf.userid);
    const fetchUserbyfolder = await this.userModel.find({ _id: { $in: userIds } });

    return { sharedfolder, fetchUserbyfolder };
  }


  async deleteShared(deleteSharedDto: DeleteSharedDto) {
        if (deleteSharedDto.docID) {
            const sharedAsset = await this.sharedModel.findOne({docid: deleteSharedDto.docID, userid: deleteSharedDto.UserID});
            if (sharedAsset) {
                await sharedAsset.deleteOne();
                return "deleted"
            }
        } else if (deleteSharedDto.foldID) {
            const sharedAsset = await this.sharedModel.findOne({folderid: deleteSharedDto.foldID, userid: deleteSharedDto.UserID});
            if (sharedAsset) {
                await sharedAsset.deleteOne();
                return "deleted"
            }
...
async findfolderbyUser(findsharedDto: findSharedDto) {
        const sharedAssets = await this.sharedModel.find({userid: findsharedDto.Userid,}).exec();
        const folderIds = sharedAssets.map(shared => shared.folderid);
        const fetchFolderbyUser = await this.folderModel.find({ _id: { $in: folderIds } });
        return fetchFolderbyUser;
}
